...
const SERVICE_KEYWORDS: Record<string, string> = {
  ...
  contract: "24-month DStv contract",
...
function botReply(text: string): Msg {
  ...
  if (/\b(hi|hello|hey|hola|howzit|sawubona|dumela)\b/.test(t)) {
    return {
      role: "bot",
      text: "Hi there! 👋 I'm Tshepo Assistant. I can help with DStv installs, WiFi setups, phone unlocking and appliance repairs. What do you need today?",
      quick: [
        { label: "DStv Install", value: "I need a DStv installation" },
        { label: "WiFi Setup", value: "I want WiFi installation" },
        { label: "Repair", value: "I need an appliance repair" },
  ...
  if (/\b(price|cost|how much|quote|charge)\b/.test(t)) {
    return {
      role: "bot",
      text: "Pricing depends on the type of service, distance and equipment needed. The fastest way to get an exact quote is via WhatsApp — our team replies within minutes.",
      quick: [{ label: "💬 Open WhatsApp", value: "__wa__" }, { label: "Call Now", value: "__call__" }],
    };
  }
  ...
  if (/\b(area|location|where|come|travel|service area)\b/.test(t)) {
    return {
      role: "bot",
      text: "We come to you anywhere in Gauteng — Pretoria, Johannesburg, Centurion and surrounding areas. Need us at a specific address?",
      quick: [{ label: "Book a Visit", value: "__wa__" }],
    };
  }
  ...
  if (/\b(time|how long|when|today|tomorrow|emergency|urgent)\b/.test(t)) {
    return {
      role: "bot",
      text: "Most installations take 1–2 hours. We offer same-day visits for urgent jobs depending on availability. Want me to check today's slots?",
      quick: [{ label: "Check Availability", value: "__wa__" }],
    };
  }
  ...
  if (/\b(warrant|guarantee)\b/.test(t)) {
    return {
      role: "bot",
      text: "Every installation comes with our Quality Installation Guarantee and 100% satisfaction promise. We don't leave until it works perfectly. ✅",
    };
  }
  ...
  if (hits.length) {
    ...
    return {
      role: "bot",
      text: `Great choice — ${svc} is one of our specialities. Tap below to chat with our technician on WhatsApp for a fast booking or quote.`,
      quick: [
        { label: "💬 WhatsApp Now", value: "__wa__" },
        { label: "📞 Call Tshepo", value: "__call__" },
      ],
    };
  }
  ...
  return {
    role: "bot",
    text: "Got it. For the quickest response, our team can help you on WhatsApp right now. Or I can answer common questions about pricing, areas, or specific services.",
    quick: [
      { label: "💬 WhatsApp", value: "__wa__" },
      { label: "Pricing", value: "How much does it cost?" },
      { label: "Service Areas", value: "What areas do you cover?" },
    ],
  };
}
...
export function ChatBot() {
  ...
  const [msgs, setMsgs] = useState<Msg[]>([
    {
      role: "bot",
      text: "👋 Welcome to Tshepo DStv! I'm your virtual assistant. How can I help you today?",
      quick: [
        { label: "DStv Install", value: "I need a DStv installation" },
        { label: "WiFi Setup", value: "I want WiFi installation" },
        { label: "Repair", value: "I need an appliance repair" },
  ...
  const handleSend = (text: string) => {
    ...
    if (!trimmed) return;
    if (trimmed === "__wa__") {
      window.open(waLink("Hi Tshepo DStv, I'd like to make an enquiry."), "_blank");
      return;
    }
    if (trimmed === "__call__") {
    ...
    <>
      ...
      {showPing && !open && (
        <div className="fixed z-40 right-4 bottom-40 lg:bottom-24 max-w-[260px] glass-card rounded-2xl p-3 reveal-up cursor-pointer" onClick={() => { setOpen(true); setShowPing(false); }}>
          <p className="text-sm font-semibold text-[color:var(--ink)]">Hi! 👋</p>
          <p className="text-xs text-[color:var(--muted-foreground)] mt-0.5">Need help? Chat with us now — instant replies!</p>
        </div>
      )}
        ...
        <div className="flex flex-col h-[78vh] sm:h-[600px] bg-white rounded-3xl shadow-2xl overflow-hidden border border-[color:var(--border)]">
          <div className="hero-gradient p-4 text-white flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="font-semibold text-sm">Tshepo Assistant</p>
